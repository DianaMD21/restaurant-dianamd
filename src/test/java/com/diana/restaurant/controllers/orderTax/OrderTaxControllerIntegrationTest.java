...
public class OrderTaxControllerIntegrationTest {
  ...
  @Test
  public void getAll_ShouldReturnAllOrderTaxs() {
    var orderTaxs = OrderTaxServiceFixtures.buildOrderTaxs(3);
    orderTaxs.stream().forEach(orderTaxController::add);
    var orderTaxsFound = orderTaxController.getAll();
    Assertions.assertEquals(orderTaxs, orderTaxsFound);
  }
  ...
  public void delete_ShouldDeleteOrderTax() {
    var orderTaxs = OrderTaxServiceFixtures.buildOrderTaxs(3);
    orderTaxs.stream().forEach(orderTaxController::add);
    var deletedOrderTax = orderTaxController.deleteById(orderTaxs.get(0).getId());
    var deletedOrderTaxToVerify = OrderTaxServiceFixtures.buildOrderTax(orderTaxs.get(0));
    Assertions.assertEquals(deletedOrderTax, orderTaxs.get(0));
