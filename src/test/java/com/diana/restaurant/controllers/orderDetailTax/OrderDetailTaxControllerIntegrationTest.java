...
public class OrderDetailTaxControllerIntegrationTest {
  ...
  @Test
  public void getAll_ShouldReturnAllOrderDetailTaxs() {
    var orderDetailTaxs = OrderDetailTaxServiceFixtures.buildOrderDetailTaxs(3);
    orderDetailTaxs.stream().forEach(orderDetailTaxController::add);
    var orderDetailTaxsFound = orderDetailTaxController.getAll();
    Assertions.assertEquals(orderDetailTaxs, orderDetailTaxsFound);
  }
  ...
  public void delete_ShouldDeleteOrderDetailTax() {
    var orderDetailTaxs = OrderDetailTaxServiceFixtures.buildOrderDetailTaxs(3);
    orderDetailTaxs.stream().forEach(orderDetailTaxController::add);
    var deletedOrderDetailTax = orderDetailTaxController.deleteById(orderDetailTaxs.get(0).getId());
    var deletedOrderDetailTaxToVerify =
        OrderDetailTaxServiceFixtures.buildOrderDetailTax(orderDetailTaxs.get(0));
    Assertions.assertEquals(deletedOrderDetailTax, orderDetailTaxs.get(0));
    Mockito.verify(orderDetailTaxServiceSpy, Mockito.times(1))
